...
- [x] **[CHORE] Rename data-types from "...Nested Content" suffix to "...Block List"** — done for the 8 data types still present after the duplicate-cleanup pass (Blog Article Expertise/Industry/Location lists, Operation Teams, Products Service Categories, Projects Project List, Quote Request Line Items, Team Member). Both `<Name>` and `Alias` updated; filename + Key kept so existing references stay valid. **Cosmetic gotcha** (2026-05-22): after uSync force-import + backoffice hard-refresh, the property-assignment chips on the About Page doctype still display "Team Member - Nested Content" — Umbraco's data-type label appears to be cached deeper than the standard cache rebuilds clear. Functional behaviour is unaffected; left as a minor display-only mismatch.
